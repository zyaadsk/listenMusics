...
class Decode : public QObject {
  Q_OBJECT
  Q_PROPERTY(QList<float> vertices READ vertices WRITE setvertices NOTIFY
                 verticesChanged)

 public:
  explicit Decode(QObject *parent = nullptr);

  Q_INVOKABLE bool decode(QString filePath, QString dir);
  void getVertic(QString filePath);
  QList<float> vertices() const { return m_vertices; }
